...
static unsigned int simple_plug_active = 0;
...
static void cpus_up_down(int desired_n_online)
{
	int cpu;
...
static void verify_cores(unsigned desired_n_cores)
{
	int cpu;
...
static void simple_plug_work_fn(struct work_struct *work)
{
#ifdef CONFIG_SIMPLE_PLUG_STATS
	if (reset_stats) {
		reset_stats = 0;
...
static void simple_plug_late_resume(struct early_suspend *handler)
{
	int i;
