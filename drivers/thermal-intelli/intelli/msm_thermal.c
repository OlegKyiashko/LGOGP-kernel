...
static uint32_t overtemp_detected = 0;
...
static int update_cpu_max_freq(int cpu, uint32_t max_freq)
{
	int ret = 0;
        ...
        // Limit to user min/max settings
        //if(!msm_cpufreq_is_in_range(cpu, max_freq))
	//	return ret;
...
static void __ref do_core_control(long temp)
{
	int i = 0;
	int ret = 0;
	uint32_t check_temp = msm_thermal_info.core_limit_temp_degC - msm_thermal_info.core_temp_hysteresis_degC;
	...
	} else if (cpus_offlined && temp <= check_temp) {
...
static void __ref do_freq_control(long temp)
{
	...
	uint32_t check_temp = msm_thermal_info.limit_temp_degC - msm_thermal_info.temp_hysteresis_degC;
...
static void __ref check_temp(struct work_struct *work)
{
	...
	long temp = 0;
	int ret = 0;
	uint32_t poll = msm_thermal_info.poll_ms;
	...
	if(overtemp_detected)
		poll >>= 1; // reduce poll to check faster when overtemp detected
...
reschedule:
	schedule_delayed_work(&check_temp_work, msecs_to_jiffies(poll));
}
                ...
                struct kobj_attribute *attr, char *buf)
{
	...
	overtemp = min(msm_thermal_info.limit_temp_degC, msm_thermal_info.core_limit_temp_degC);

	warning = min((msm_thermal_info.limit_temp_degC - msm_thermal_info.temp_hysteresis_degC),
		      (msm_thermal_info.core_limit_temp_degC - msm_thermal_info.core_temp_hysteresis_degC));
...
static int __ref update_offline_cores(int val)
{
	...
	if (!core_control_enabled && !cpus_offlined)
		return 0;
